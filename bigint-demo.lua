...
function demo_serialize_unserialize(n)
    print(string.format("bigint.unserialize(bigint.new(%s)) -> %s", tostring(n), tostring(bigint.unserialize(bigint.new(n), true))))
end
...
function demo_exponentiate(n1, n2)
    print(string.format("bigint.multiply(%s, %s) -> %s", tostring(n1), tostring(n2), tostring(bigint.unserialize(bigint.exponentiate(bigint.new(n1), n2), true))))
end
...
print("-- Raise a positive to a positive integer")
demo_exponentiate(math.random(100), math.random(100))
print("-- Raise a negative to a positive integer")
